...
async function postDataDownloadBlobLocal(
  path: string,
  body: any = {}
): Promise<any> {
  let url = completeLocalUrl(path);
...
export async function postDataDownloadBlob(
  path: string,
  body: any = {}
): Promise<any> {
  let result = undefined;
  if (isRunningOnCloud()) {
    result = await API.post(backendAPI, path, { body, responseType: "blob" });
  } else {
    result = await postDataDownloadBlobLocal(path, body);
  }
  return result;
}
